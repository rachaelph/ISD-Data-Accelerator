...
def get_dq_by_log_id_sql(log_id: str) -> str:
    return (
        ...
        "Ingestion_End_Time, Job_Run_URL FROM Data_Quality_Notifications "
...
def get_activity_logs_by_log_id_sql(log_id: str) -> str:
    return (
        ...
        "FROM Activity_Run_Logs WHERE Log_ID = '{log_id}' ORDER BY Sequence_Number ASC;"
    ).format(log_id=sql_string_literal(log_id))
...
def main() -> int:
    ...
    try:
        ...
        if args.query_type == "FullHealth":
            ...
            metadata_queries = [
                ("Orchestration", f"SELECT Trigger_Name, Order_Of_Operations, Table_ID, Target_Datastore, Target_Entity, Primary_Keys, Processing_Method, Ingestion_Active FROM Data_Pipeline_Metadata_Orchestration WHERE Table_ID = {args.table_id};"),
            ...
            stale_subquery = (
                f"SELECT Log_ID FROM Data_Pipeline_Logs "
            ...
            if args.resolve or args.delete:
                ...
                select_sql = (
                    ...
                    f"FROM Data_Pipeline_Logs {stale_filter} "
            ...
            if args.resolve:
                ...
                insert_sql = (
                    f"INSERT INTO Data_Pipeline_Logs "
            ...
            escaped_trigger = sql_string_literal(args.trigger_name)
            overview_queries = [
                ("Orchestration", f"SELECT Trigger_Name, Order_Of_Operations, Table_ID, Target_Datastore, Target_Entity, Primary_Keys, Processing_Method, Ingestion_Active FROM Data_Pipeline_Metadata_Orchestration WHERE Trigger_Name = '{escaped_trigger}' ORDER BY Order_Of_Operations, Table_ID;"),
                ("LastRunPerTable", (
                    ...
                    f"FROM Data_Pipeline_Logs WHERE Processing_Phase = 'Batch' ) l "
