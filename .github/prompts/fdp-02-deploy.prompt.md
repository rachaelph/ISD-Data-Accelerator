---
agent: agent
description: "Deploy the accelerator IP into a Databricks workspace and guide the first-time setup steps that make the deployed workspace usable with Git, local cloning, and ongoing accelerator development"
---

# Deploy And Set Up Accelerator IP

Deploy the Databricks Data Platform Accelerator into a Databricks workspace, then guide the required first-time setup steps for: **{{ deployment_context }}**
...
└── Call out any blockers early (missing Databricks access, missing repo access, missing notebooks, or unclear working-repo structure)
...
├── List the notebooks that must be imported into Databricks
├── Explain the deployment parameters the user needs to set in CreatingArtifacts_Orchestrator.ipynb
├── Explain how the working repository should be set up after deployment, including local clone and Databricks Git integration
...
├── Tell the user to run CreatingArtifacts_Orchestrator.ipynb in Databricks
├── Explain what artifacts will be created
├── Explain how to move the accelerator support folders into the cloned working repo
├── Warn the user not to copy the accelerator repo's artifact folder into the target repo, because that will conflict with the target repo's real Databricks artifact tree and confuse Copilot automation
├── Use the target repo's actual top-level Databricks artifact folder in guidance, typically the folder connected to the workspace in Databricks Git integration and often named after the workspace
...
- Use the target repo's actual top-level Databricks artifact folder in user-facing guidance. Do not assume a fixed folder name.
...
- Be explicit that the accelerator repo's artifact folder should not be copied into the target repo. The target repo's own artifact folder must remain the only Databricks artifact root so Copilot automation resolves the correct files.
